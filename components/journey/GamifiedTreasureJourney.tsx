...
export default function GamifiedTreasureJourney() {
 ...
 const [shareUrl, setShareUrl] = useState('https://hackerhousegoa.com');

 useEffect(() => {
 setShareUrl(`${window.location.origin}/card/${profile.cardId ?? ''}`);
 }, [profile.cardId]);
 ...
 setUnlockedStep(4);
 triggerToast(' UNLOCKED: Goan Beach Lounge Chair!');
 setTimeout(() => {
 ...
 onClick={() => {
 setMapUnlocked(true);
 triggerToast(" Map Acquired! The journey begins...");
 setTimeout(() => {
 ...
 <span style={{ fontSize: 11, color: 'var(--goa-yellow)', opacity: 0.8 }}>REWARD: Beach Lounge Chair</span>
 )}
 </div>
 ...
 >
 {earnedItems.beachChair ? '✓ BEACH CHAIR EARNED — UNLOCK MYSTERY CHEST ↓' : 'CLAIM BEACH CHAIR & UNLOCK MYSTERY CHEST ↓'}
 </button>
 </motion.div>
